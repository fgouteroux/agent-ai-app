...
func TestSanitizePrompt_WhitespaceOnlyReducesToEmpty(t *testing.T) {
	t.Parallel()

	for _, input := range []string{"   ", "\t\t", "\n\n", "  \t \n "} {
		got := sanitizePrompt(input)
		if got != "" {
			t.Errorf("sanitizePrompt(%q) = %q, want empty string", input, got)
		}
	}
}

func TestSanitizePrompt_TrimsSurroundingWhitespace(t *testing.T) {
	t.Parallel()

	got := sanitizePrompt("  hello world  ")
	if got != "hello world" {
		t.Errorf("sanitizePrompt(%q) = %q, want %q", "  hello world  ", got, "hello world")
	}
}
